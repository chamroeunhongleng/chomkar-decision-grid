...
### The calibration loop, run for real

The seed ledger ([data/outcomes.csv](data/outcomes.csv)) has 3 recorded deliveries. Running the
calibration tool against it — deterministically, no fixture needed —

```bash
py tools/calibrate_weights.py
```

produces this actual output:

```
[Calibrate] 3 outcomes, mean success 0.88
      factor |   corr |  current | proposed
    coverage |    0.0 |     0.35 |   0.3313
       price | -0.866 |      0.3 |    0.243
       route |  0.999 |      0.2 |   0.2365
   freshness |  0.999 |     0.15 |   0.1892

  PROPOSAL ONLY — a human approves by editing docs/business_rules.md §3 and config.GRID_WEIGHTS.
```

Read literally: across these 3 deliveries, route safety and freshness tracked success almost perfectly
(+0.999 correlation each) while price headroom correlated *negatively* (-0.866) — the cheapest lot in
the ledger (`order_h002`) was the one that arrived late and partly spoiled. The tool proposes moving
weight from price toward route/freshness accordingly — bounded to ±0.05 per factor, renormalized to
sum to 1.0. It writes `outputs/calibration_proposal.json` (gitignored, regenerated by the tool — see
the command above) and stops there: nothing is auto-applied. A human decides whether 3 data points are
enough evidence to edit
`docs/business_rules.md` §3, per [AGENTS.md](AGENTS.md) rule 8. Record more outcomes with `/outcome`
and re-run to sharpen (or overturn) this proposal.
